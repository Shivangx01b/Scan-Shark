...
def exploitdb_tcp_search(file1,o):
   with open('nmap_tcp_scan.txt') as open_tcp2:
    ...
    for port_range in range(1, 65535):
      if str(str(port_range) + '/tcp')  in r8:
        ...
        if len(query3) != 0:
          ...
          r9 = []
          for result4 in search(query, tld="co.in", num=20, start=0, stop=25, pause=2):
...
def exploitdb_udp_search(file2,o):
   with open('nmap_udp_scan.txt') as open_udp2:
    ...
    for port_range in range(1, 65535):
      if str(str(port_range) + '/udp')  in r11:
        ...
        if len(query4) != 0:
          ...
          r12 = []
          for result5 in search(query, tld="co.in", num=20, start=0, stop=25, pause=2):
